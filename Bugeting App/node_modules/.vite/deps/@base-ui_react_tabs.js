...
function useCompositeItem(params = {}) {
  ...
  const compositeProps = React3.useMemo(() => ({
...
var TabsListContext = React4.createContext(void 0);
if (true) TabsListContext.displayName = "TabsListContext";
function useTabsListContext() {
  const context = React4.useContext(TabsListContext);
  if (context === void 0) {
...
// node_modules/@base-ui/react/esm/tabs/tab/TabsTab.js
var TabsTab = React5.forwardRef(function TabsTab2(componentProps, forwardedRef) {
  const {
    className,
    disabled = false,
    render,
  ...
  const tabMetadata = React5.useMemo(() => ({
    disabled,
    id,
    value
  }), [disabled, id, value]);
  ...
  const isNavigatingRef = React5.useRef(false);
  const tabElementRef = React5.useRef(null);
  React5.useEffect(() => {
  ...
  const isPressingRef = React5.useRef(false);
  const isMainButtonRef = React5.useRef(false);
...
var TabsList = React10.forwardRef(function TabsList2(componentProps, forwardedRef) {
  ...
  const [tabsListElement, setTabsListElement] = React10.useState(null);
  const indicatorUpdateListenersRef = React10.useRef(/* @__PURE__ */ new Set());
  const tabResizeObserverElementsRef = React10.useRef(/* @__PURE__ */ new Set());
  const resizeObserverRef = React10.useRef(null);
...
function useActivationDirectionDetector(activeTabValue, orientation, tabsListElement, getTabElement) {
  const [previousTabEdge, setPreviousTabEdge] = React10.useState(null);
  useIsoLayoutEffect(() => {
